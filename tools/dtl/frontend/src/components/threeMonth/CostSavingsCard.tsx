...
function perDieCostSaving(agg: CostSavingsAggregate, ratePerHour: number): number {
  return (agg.total_estimated_seconds_saved / 3600) * ratePerHour;
}

function CostSavingsConfigPanel({
  config,
  onConfigChange,
  onPreviewRate,
}: {
  config: CostSavingsConfig;
  onConfigChange: (patch: Partial<CostSavingsConfig>) => void;
  onPreviewRate: (rate: number) => void;
}) {
  const [draft, setDraft] = useState(String(config.testerCostPerHour));
  const inputRef = useRef<HTMLInputElement>(null);
  const fieldClass =
    "w-full max-w-xs rounded border border-[var(--border-subtle)] bg-[var(--bg-panel)] px-2.5 py-1.5 text-sm font-mono text-[var(--text-primary)] focus:border-[var(--accent)] focus:outline-none";

  useEffect(() => {
    if (document.activeElement === inputRef.current) return;
    setDraft(String(config.testerCostPerHour));
  }, [config.testerCostPerHour]);

  const commitDraft = () => {
    const trimmed = draft.trim();
    if (!trimmed) {
      setDraft(String(config.testerCostPerHour));
      onPreviewRate(config.testerCostPerHour);
      return;
    }
    ...
    onConfigChange({ testerCostPerHour: parsed });
    onPreviewRate(parsed);
  };

  return (
    <div
      className="mt-3 rounded-md border border-amber-500/25 bg-amber-500/5 p-3"
      data-testid="cost-savings-config"
    >
      <label className="block text-xs text-[var(--text-secondary)]">
        <span className="mb-1 block font-semibold uppercase tracking-wide text-amber-600 dark:text-amber-400">
        ...
        <input
          ...
          value={draft}
          data-testid="cost-config-tester-rate"
          onChange={(e) => {
...
export function PredictedCostSavingsCard({
  ...
  const [previewRate, setPreviewRate] = useState(config.testerCostPerHour);

  useEffect(() => {
    setPreviewRate(config.testerCostPerHour);
  }, [config.testerCostPerHour]);
  ...
  if (loading) {
    return (
      ...
      >
        ...
        <CostSavingsConfigPanel
          config={config}
          onConfigChange={onConfigChange}
          onPreviewRate={setPreviewRate}
        />
  ...
  const perDieSaving = perDieCostSaving(agg, previewRate);
  const moneyFormatted = money(perDieSaving);
      ...
      <details className="mt-4 rounded-md border border-[var(--border-subtle)] bg-[var(--bg-panel-secondary)] p-3 text-xs text-[var(--text-secondary)]" data-testid="cost-savings-explanation">
        ...
        <div className="mt-3 space-y-3 border-t border-[var(--border-subtle)] pt-3">
          <dl className="space-y-2">
            ...
            <div>
              <dt className="font-semibold text-amber-600 dark:text-amber-400">ATE burden rate</dt>
              <dd className="text-[var(--text-secondary)] mt-0.5">
                Money saved scales with your configured ATE burden rate (${config.testerCostPerHour}
                /hr). Test time estimates come from the DTL counterfactual model.
              </dd>
            </div>
          </dl>
