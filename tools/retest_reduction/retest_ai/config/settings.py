...
RETEST_DATA_DIR = os.path.join(REPO_ROOT, "data", "retest_reduction")
ARTIFACTS_DIR = os.path.join(BASE_DIR, "artifacts")
os.makedirs(ARTIFACTS_DIR, exist_ok=True)


def _is_pre_retest_workbook(name: str) -> bool:
    lowered = name.lower()
    return "pre_retest" in lowered or "pre-retest" in lowered


def _is_outcomes_workbook(name: str) -> bool:
    lowered = name.lower()
    if _is_pre_retest_workbook(lowered):
        return False
    return any(
        key in lowered
...
def _resolve_month_12_outcomes_file() -> str:
    """Resolve post-retest outcomes workbook for validation-only loading."""
    explicit = os.environ.get("RETEST_OUTCOMES_FILE")
    if explicit and os.path.isfile(explicit):
        return explicit

    candidates: list[str] = [
        os.path.join(RETEST_DATA_DIR, "outcomes.xlsx"),
        os.path.join(RETEST_DATA_DIR, "Month_12_PRIVATE_VALIDATION_ONLY.xlsx"),
        os.path.join(WORKSPACE_DIR, "Month_12_PRIVATE_VALIDATION_ONLY.xlsx"),
        os.path.join(os.path.dirname(WORKSPACE_DIR), "Month_12_PRIVATE_VALIDATION_ONLY.xlsx"),
    ]

    data_dir = Path(RETEST_DATA_DIR)
    if data_dir.is_dir():
        for path in sorted(data_dir.glob("*.xlsx")):
            if _is_outcomes_workbook(path.name):
    ...
    return os.path.join(RETEST_DATA_DIR, "outcomes.xlsx")
...
MONTH_12_OUTCOMES_FILE = _resolve_month_12_outcomes_file()
